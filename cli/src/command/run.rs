...
pub async fn run(alias: &str, args: &[String], bin: Option<&str>, registry: &str) -> Result<()> {
    ...
    // Validate that the binary name is a plain filename with no path components.
    // This prevents path traversal attacks via --bin (e.g., "../../etc/passwd"
    // or "/etc/passwd") that could escape the artifact output directory.
    ...
    debug!("run: executing {}", binary_path.display());
