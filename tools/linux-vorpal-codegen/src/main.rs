...
fn generate_go_script(
    ...
    let go_func_name = to_go_pascal(func_name);
...
fn generate_go_source(sources: &[ParsedSourceFn]) -> String {
    ...
    for src in sources {
        ...
        out.push_str(&format!(
            "\treturn artifact.NewArtifactSource({}, path).Build()\n",
            name_ref
        ));
        out.push_str("}\n");
    }
...
/// Convert source function name to Go PascalCase, handling special cases.
...
fn generate_go_orchestration(
    ...
    let mut out = String::from(
        "// Code generated by linux-vorpal-codegen. DO NOT EDIT.\n\n\
         ...
         func Build(ctx *config.ConfigContext) (*string, error) {\n",
    );
    ...
    // LinuxDebian rootfs
    out.push_str(
        "\tstepRootfs, err := artifact.NewLinuxDebian().Build(ctx)\n\
         \tif err != nil {\n\
         \t\treturn nil, err\n\
         \t}\n\n",
    );
    ...
    out.push_str(
        "\tname := \"linux-vorpal\"\n\n\
         \treturn artifact.NewArtifact(name, steps, systems).\n\
...
fn generate_go_script_calls(versions: &[VersionConst]) -> String {
    ...
    // Setup script
    out.push_str("\tstepSetupScript := Setup(\n");
    for param in &[
        "binutils_version",
        "gawk_version",
        "gcc_version",
    ...
    // Stage 01 script
    out.push_str("\tstepStage01Script := Stage01(\n");
    for param in &[
        "binutils_version",
        "gcc_version",
        "glibc_version",
    ...
    // Stage 02 script
    out.push_str("\tstepStage02Script := Stage02(\n");
    for param in &[
        "bash_version",
        "binutils_version",
        "coreutils_version",
    ...
    // Stage 03 script
    out.push_str("\tstepStage03Script := Stage03(\n");
    for param in &[
        "bison_version",
        "gettext_version",
        "perl_version",
    ...
    // Stage 04 script
    out.push_str("\tstepStage04Script := Stage04(\n");
    for param in &[
        "binutils_version",
        "gcc_version",
        "glibc_version",
    ...
    // Stage 05 script
    out.push_str("\tstepStage05Script := Stage05(\n");
    for param in &[
        "curl_version",
        "libidn2_version",
        "libpsl_version",
...
fn main() {
    ...
    let go_output_dir = project_root.join("sdk/go/pkg/artifact/linux_vorpal");
    ...
    write_or_check(
        &go_output_dir.join("linux_vorpal.go"),
        &go_orchestration,
        check_mode,
        &mut mismatches,
    );
