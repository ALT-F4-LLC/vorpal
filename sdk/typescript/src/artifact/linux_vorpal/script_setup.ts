...
export function setup(binutilsVersion: string, gawkVersion: string, gccVersion: string, glibcVersion: string, gmpVersion: string, mpcVersion: string, mpfrVersion: string, ncursesVersion: string): string {
  return `set +h
umask 022
...
pushd \$VORPAL_SOURCE/gcc/gcc-${gccVersion}

mv \$VORPAL_SOURCE/mpfr/mpfr-${mpfrVersion} mpfr
mv \$VORPAL_SOURCE/gmp/gmp-${gmpVersion} gmp
mv \$VORPAL_SOURCE/mpc/mpc-${mpcVersion} mpc
...
pushd \$VORPAL_SOURCE/gawk/gawk-${gawkVersion}
...
pushd \$VORPAL_SOURCE/glibc/glibc-${glibcVersion}
...
pushd \$VORPAL_SOURCE/binutils-pass-02/binutils-${binutilsVersion}
...
pushd \$VORPAL_SOURCE/gcc-pass-02/gcc-${gccVersion}
