...
func Rustc(context *config.ConfigContext) (*string, error) {
	name := "rustc"
	...
	stepScript := fmt.Sprintf(`cp -pr "./source/%s/%s-%s-%s/%s/." "$VORPAL_OUTPUT"`, name, name, sourceVersion, *sourceTarget, name)
