...
func linuxVorpalStage04(binutilsVersion string, gccVersion string, glibcVersion string, opensslVersion string, zlibVersion string) string {
...
mkdir -p $VORPAL_SOURCE/glibc-pass-02/glibc-%s/build
pushd $VORPAL_SOURCE/glibc-pass-02/glibc-%s/build
...
mkdir -p $VORPAL_SOURCE/zlib/zlib-%s/build
pushd $VORPAL_SOURCE/zlib/zlib-%s/build
...
mkdir -p $VORPAL_SOURCE/binutils-pass-03/binutils-%s/build
pushd $VORPAL_SOURCE/binutils-pass-03/binutils-%s/build
...
mkdir -p $VORPAL_SOURCE/gcc-pass-03/gcc-%s/build
pushd $VORPAL_SOURCE/gcc-pass-03/gcc-%s/build
...
chown -R root:root /usr/lib/gcc/$(gcc -dumpmachine)/15.2.0/include{,-fixed}

ln -sr /usr/bin/cpp /usr/lib
ln -s gcc.1 /usr/share/man/man1/cc.1
ln -sf ../../libexec/gcc/$(gcc -dumpmachine)/15.2.0/liblto_plugin.so /usr/lib/bfd-plugins/
...
mkdir -p $VORPAL_SOURCE/openssl/openssl-%s/build
pushd $VORPAL_SOURCE/openssl/openssl-%s/build
...
rm -rf $VORPAL_SOURCE/openssl`, glibcVersion, glibcVersion, zlibVersion, zlibVersion, binutilsVersion, binutilsVersion, gccVersion, gccVersion, opensslVersion, opensslVersion)
}
