...
fn to_py_name(s: &str) -> String {
    ...
    if py_reserved.contains(&name.as_str()) {
        name.push('_');
    }
    name
}
...
/// Generate a Python script function file.
fn generate_py_script(
    func_name: &str,
    template: &str,
    params: &[String],
    named_args: &[(String, String)],
    ...
    let body = build_py_f_string_body(template, &named_arg_map);
    let param_list = params
        .iter()
        .map(|p| format!("{}: str", to_py_name(p)))
        .collect::<Vec<_>>()
        .join(", ");
    let py_func_name = to_py_name(func_name);

    format!(
        concat!(
            "# Code generated by linux-vorpal-codegen. DO NOT EDIT.\n\n",
            "from __future__ import annotations\n\n\n",
            "def {py_func_name}({param_list}) -> str:\n",
            "    return rf\"\"\"{body}\"\"\"\n",
        ),
...
fn build_py_f_string_body(template: &str, named_arg_map: &HashMap<&str, &str>) -> String {
    let mut result = String::new();
    let chars: Vec<char> = template.chars().collect();
    let mut i = 0;
...
/// Transform a Rust expression to Python equivalent.
...
/// Generate Python source.py from parsed source functions.
fn generate_py_source(sources: &[ParsedSourceFn]) -> String {
    let mut out = String::from(
        "# Code generated by linux-vorpal-codegen. DO NOT EDIT.\n\n\
         from __future__ import annotations\n\n\
    ...
    for src in sources {
        out.push('\n');
        let py_func_name = source_func_name_to_py(&src.name);
        let param_list = src
            .params
            .iter()
            .map(|p| format!("{}: str", to_py_name(p)))
        ...
        if src.has_version_replace {
            out.push_str("    version_clean = version.replace(\".\", \"\")\n");
        }

        let py_path = build_py_source_path(&src.path_template, src.has_version_replace);
        let name_ref = if src.source_name_is_param {
            to_py_name(&src.source_name)
        } else {
            "name".to_string()
        };
...
/// Build a Python f-string for a source path URL.
fn build_py_source_path(template: &str, has_version_replace: bool) -> String {
    if template.is_empty() {
        return "\"\"".to_string();
    }
...
fn generate_ts_orchestration(
    ...
    // Systems
    out.push_str("  const systems = [\"aarch64-linux\", \"x86_64-linux\"];\n\n");
...
fn generate_py_orchestration(
    ...
    let source_func_names: Vec<String> = source_calls
        .iter()
        .map(|sc| source_func_name_to_py(&sc.func_name))
        .collect::<std::collections::HashSet<_>>()
        .into_iter()
        .collect();
    ...
    out.push_str("    step_environments = [\"PATH=/usr/bin:/usr/sbin\"]\n\n");
    out.push_str("    step_rootfs = LinuxDebian().build(context)\n\n");
    out.push_str(&generate_py_script_calls());
    out.push('\n');
    ...
    for step in steps {
        let bwrap_args = match step.bwrap_mode.as_str() {
            ...
            "bwrap_arguments" => "bwrap_arguments".to_string(),
            _ => "[]".to_string(),
        };
...
/// Build Python argument string for a source call.
fn build_py_source_args(args: &[String], versions: &[VersionConst]) -> String {
    args.iter()
        .map(|a| {
            let trimmed = a.trim();
            if versions.iter().any(|v| v.name == trimmed) || trimmed.ends_with("_version") {
...
/// Generate Python script function calls.
fn generate_py_script_calls() -> String {
    let mut out = String::new();

    out.push_str("    step_setup_script = setup(\n");
    for param in &[
        "binutils_version",
        "gawk_version",
        "gcc_version",
    ...
    out.push_str("    step_stage_01_script = stage_01(\n");
    for param in &[
        "binutils_version",
        "gcc_version",
        "glibc_version",
    ...
    out.push_str("    step_stage_02_script = stage_02(\n");
    for param in &[
        "bash_version",
        "binutils_version",
        "coreutils_version",
    ...
    out.push_str("    step_stage_03_script = stage_03(\n");
    for param in &[
        "bison_version",
        "gettext_version",
        "perl_version",
    ...
    out.push_str("    step_stage_04_script = stage_04(\n");
    for param in &[
        "binutils_version",
        "gcc_version",
        "glibc_version",
    ...
    out.push_str("    step_stage_05_script = stage_05(\n");
    for param in &[
        "curl_cacert_version",
        "curl_version",
        "libidn2_version",
...
fn generate_py_scripts_module(script_files: &[(&str, &str)]) -> String {
    let mut out = String::from("# Code generated by linux-vorpal-codegen. DO NOT EDIT.\n\n");
...
fn generate_py_init_module() -> String {
    "# Code generated by linux-vorpal-codegen. DO NOT EDIT.\n\n\
...
fn main() {
    ...
    let py_output_dir = project_root.join("sdk/python/src/vorpal_sdk/artifact/linux_vorpal");
    ...
    write_or_check(
        &py_output_dir.join("linux_vorpal.py"),
        &py_orchestration,
        check_mode,
        &mut mismatches,
    );
    ...
    write_or_check(
        &py_output_dir.join("__init__.py"),
        &py_init,
        check_mode,
        &mut mismatches,
    );
