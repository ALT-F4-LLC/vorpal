"""Linux Debian rootfs builder used by the linux-vorpal bootstrap."""
...
def _generate_version_script() -> str:
    return """#!/bin/bash
...
def _generate_dockerfile() -> str:
    return (
        "FROM docker.io/library/debian:sid-slim@sha256:"
        "c0f1b3716686ee452f7c62c82d8aee5f79feccba7402e967b79658100d5bd6cf\n"
        "\n"
        "RUN ARCH=$(uname -m) && if [ \"${ARCH}\" = \"aarch64\" ]; then "
        "ARCH=\"arm64\"; fi && if [ \"${ARCH}\" = \"x86_64\" ]; then "
        "ARCH=\"amd64\"; fi && echo \"Current architecture: ${ARCH}\" && "
        "apt-get update && apt-get install --yes bash binutils bison bubblewrap "
        "bzip2 ca-certificates coreutils curl diffutils g++ gawk gcc grep gzip "
        "linux-headers-$ARCH m4 make patch perl python3 rsync sed tar texinfo "
        "xz-utils zstd && rm -rf /var/lib/apt/lists/*\n"
        "\n"
        "RUN ln -sf /bin/bash /bin/sh && [ ! -e /etc/bash.bashrc ] || mv "
        "/etc/bash.bashrc /etc/bash.bashrc.NOUSE && groupadd --gid 1000 vorpal "
...
class LinuxDebian:
    ...
    def build(self, context: ConfigContext) -> str:
        ...
        dockerfile_digest = Artifact(
            "linux-debian-dockerfile",
            [bash([], [], [], script)],
            systems,
        ).build(context)

        image = f"altf4llc/debin:{dockerfile_digest}"
        ...
        steps = [
            docker(
                [
                    ...
                    "--progress=plain",
                    f"--tag={image}",
                    get_env_key(dockerfile_digest),
                ],
                [dockerfile_digest],
            ),
            docker(["container", "create", "--name", dockerfile_digest, image], []),
            docker(
                [
                    "container",
                    "export",
            ...
            docker(["container", "stop", dockerfile_digest], []),
            docker(["container", "rm", "--force", dockerfile_digest], []),
        ]
