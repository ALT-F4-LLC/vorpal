...
/// Build-target `clippy` toolchain component, sourced from the pinned Rust toolchain release.
#[derive(Default)]
pub struct Clippy {}

impl Clippy {
    /// Creates a builder for the `clippy` toolchain component.
    #[must_use]
    pub fn new() -> Self {
        Self::default()
    }

    /// Builds the `clippy` artifact for the context's target system.
    ///
    /// # Errors
    ///
