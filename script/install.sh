...
#   VORPAL_VERSION=<ver>       Version to install (default: 0.3.0)
...
VORPAL_VERSION="${VORPAL_VERSION:-0.3.0}"
...
Options:
  ...
  -v, --version <ver>    Version to install (default: 0.3.0)
...
resolve_version() {
    ...
    if [[ "$version" = "latest" ]]; then
        ...
        if [[ "$http_code" = "403" ]]; then
            print_warning "GitHub API rate limit reached. Falling back to 0.3.0."
            version="0.3.0"
            ...
            if [[ -n "$api_response" ]]; then
                ...
                if [[ -n "$tag" ]]; then
                    ...
                    print_warning "Could not parse latest version from GitHub API. Falling back to 0.3.0."
                    version="0.3.0"
                fi
            else
                print_warning "Could not fetch latest version from GitHub API. Falling back to 0.3.0."
                version="0.3.0"
            fi
        else
            print_warning "GitHub API returned HTTP ${http_code}. Falling back to 0.3.0."
            version="0.3.0"
        fi
    fi
    ...
    ${_sym_bullet} latest  -- most recent stable release" \
            "Or specify an exact tag: --version v0.3.0
